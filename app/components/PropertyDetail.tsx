  ...
  return (
    <div style={{ background: "rgb(var(--bg))", minHeight: "100vh", color: "rgb(var(--ink))", fontFamily: "'Jost', sans-serif" }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(var(--bg),0.96)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(var(--accent),0.1)", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: 8, color: "rgba(var(--ink),0.5)", textDecoration: "none", fontSize: "0.8rem", letterSpacing: "0.1em", transition: "color 0.3s" }}
        ...
        <div style={{ display: "flex", border: "1px solid rgba(var(--accent),0.25)", borderRadius: 999, padding: 3, gap: 2 }}>
          {(["en", "es"] as const).map(l => (
            <button key={l} onClick={() => setLang(l)} style={{ padding: "3px 12px", borderRadius: 999, border: "none", cursor: "pointer", fontFamily: "'Jost', sans-serif", fontSize: "0.65rem", letterSpacing: "0.2em", textTransform: "uppercase", transition: "all 0.3s", background: lang === l ? "rgb(var(--accent))" : "transparent", color: lang === l ? "#FAF6EE" : "rgba(var(--ink),0.4)", fontWeight: lang === l ? 600 : 400 }}>
              {l}
            </button>
          ))}
        </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "60px 24px", display: "grid", gridTemplateColumns: "1fr", gap: 48 }} className="listing-layout">
        ...
        <div>
            ...
            <div style={{ display: "flex", gap: 24 }}>
              {p.specs.map(({ icon, val, label }, i) => {
                ...
                <div key={i} style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.58rem", letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(var(--ink),0.35)", marginTop: 2 }}>{tr(label)}</div>
                </div>
                );
              })}
            </div>
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <p style={{ color: "rgba(var(--ink),0.55)", lineHeight: 1.85, fontSize: "0.92rem" }}>
              {tr(p.description)}
            </p>
          </div>
          ...
          {p.amenities.length > 0 && (
            <div style={{ marginBottom: 40 }}>
              <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.4rem", fontWeight: 300, color: "rgb(var(--ink))", marginBottom: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
                {p.amenities.map(({ icon, label }, i) => {
                  const Icon = ICON_MAP[icon];
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", border: "1px solid rgba(var(--accent),0.1)", background: "rgba(var(--ink),0.03)" }}>
                      <Icon size={15} color="rgb(var(--accent))" style={{ flexShrink: 0 }} />
                      <span style={{ fontSize: "0.85rem", color: "rgba(var(--ink),0.6)" }}>{tr(label)}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10 }}>
              {p.highlights.map((h, i) => (
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  <Check size={14} color="rgb(var(--accent))" style={{ flexShrink: 0, marginTop: 2 }} />
                  <span style={{ fontSize: "0.85rem", color: "rgba(var(--ink),0.55)", lineHeight: 1.5 }}>{tr(h)}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
              {p.nearby.map((item, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <span style={{ fontSize: "0.88rem", color: "rgba(var(--ink),0.55)" }}>{tr(item.text)}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ borderTop: "1px solid rgba(var(--accent),0.1)", paddingTop: 24 }}>
              ...
              {sent ? (
                <div style={{ textAlign: "center", padding: "20px 0" }}>
                  ...
                  <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.2rem", color: "rgb(var(--ink))" }}>
