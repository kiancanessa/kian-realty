...
import type { Sale, SaleContent, SaleLocaleContent } from "../../lib/sales";
import { EMPTY_SALE_CONTENT } from "../../lib/sales";
...
export function emptySaleForm(): SaleFormState {
  return {
    content: { en: { ...EMPTY_SALE_CONTENT.en }, es: { ...EMPTY_SALE_CONTENT.es } },
    image_url: "",
  };
}

export function saleToForm(s: Sale): SaleFormState {
  return { content: s.content, image_url: s.image_url ?? "" };
}

export default function SaleEditor({
  initial, onSave, onCancel, saving,
}: {
  initial: SaleFormState;
  onSave: (form: SaleFormState) => void;
  onCancel: () => void;
  saving: boolean;
}) {
  const [form, setForm] = useState<SaleFormState>(initial);
  const [locale, setLocale] = useState<Locale>("en");

  const updateField = (l: Locale, key: keyof SaleLocaleContent, value: string) => {
        ...
        <div style={{ width: "min(480px, 100%)", border: "1px solid rgba(var(--accent),0.1)", background: "rgb(var(--surface))", overflow: "hidden" }}>
          ...
          <div style={{ padding: 24 }}>
            <Editable value={c.title} onChange={v => updateField(locale, "title", v)}
              placeholder={locale === "es" ? "Título de la propiedad" : "Property title"}
            ...
            <Editable value={c.description} onChange={v => updateField(locale, "description", v)} multiline
              placeholder={locale === "es" ? "Descripción de la venta" : "Sale description"}
            ...
            <div style={{ borderTop: "1px solid rgba(var(--accent),0.1)", paddingTop: 12 }}>
              ...
              <Editable value={c.results} onChange={v => updateField(locale, "results", v)}
                placeholder={locale === "es" ? "ej. Vendida en 45 días · $450,000 USD" : "e.g. Sold in 45 days · $450,000 USD"}
